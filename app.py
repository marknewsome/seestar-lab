...
@app.route("/catalog/<catalog_type>/poster")
def catalog_poster(catalog_type: str):
...
def api_thumbnail(object_name: str):
    ...
    # Prefer the user-pinned thumbnail; fall back to the auto-selected one.
    thumb_path = None
...
@app.route("/api/session/<path:object_name>/images")
def api_session_images(object_name: str):
    """Return all image files found in the session's directories."""
    sessions_list = db.get_all_sessions()
    ...
    for dir_path in session.get("paths", []):
        try:
            for fname in sorted(os.listdir(dir_path)):
                if fname.startswith("."):
                    continue
                if _Path(fname).suffix.lower() in IMAGE_EXT:
            ...
            pass
    return jsonify({"images": images, "pinned": session.get("pinned_thumbnail")})


@app.route("/api/session/<path:object_name>/pin-thumbnail", methods=["POST"])
def api_pin_thumbnail(object_name: str):
    """Set or clear the user-pinned thumbnail for a session."""
    sessions_list = db.get_all_sessions()
    session = next((s for s in sessions_list if s["object_name"] == object_name), None)
    if not session:
        abort(404)
    body = request.get_json(silent=True) or {}
    path = body.get("path")  # None clears the pin
    if path and not os.path.isfile(str(path)):
        return jsonify({"error": "file not found"}), 400
    db.set_pinned_thumbnail(object_name, path or None)
    # Reload and broadcast so the card thumbnail updates live
    sessions_list = db.get_all_sessions()
